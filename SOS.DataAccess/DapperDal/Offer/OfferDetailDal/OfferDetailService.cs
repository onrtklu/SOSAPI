...
namespace SOS.DataAccess.DapperDal.Offer.OfferDetailDal
{
    public class OfferDetailService : DapperGenericRepository<OfferDetail>, IOfferDetailService
    {
        ...
        {
            var result = _connection.QuerySingle<bool>(
                "Offer.IsMenuItemAdded",
                new {
        ...
        {
            var result = _connection.QuerySingle<MenuItemDto>(
                "Offer.GetMenuItem",
                new {
                        MenuItem_Id = menuItemId,
                        Customer_Id = customerId
        ...
        {
            var item = _connection.Query<OfferMenuItemList>(
                "Offer.GetOfferMenuItemList",
                new {
                        Customer_ID = customerId,
        ...
        public void OfferDetailDelete(int offer_Id, int menuItem_Id)
        {
            this.DeleteMultiple(
                s => 
                s.OfferId == offer_Id && 
                s.MenuItemId == menuItem_Id
            );

            int offerDetailCount = _connection.ExecuteScalar<int>(
                "SELECT COUNT(Id) " +
                "FROM Offer.OfferDetail " +
